...
if (!-e 'alignment.sorted.bam'){
 ...
 #  system("$bwa_bin aln -t $thread_no -n 0.1 -o 5 -e 5 -d 5 -i 3 -k 4 -R 22  hg19+virus.fa $unmappedfq1 > unmapped.1.sai 2> /dev/null");

   if ($paired){
       system("$bwa_bin mem -M -t $thread_no hg19+virus.fa $unmappedfq1 $unmappedfq2 > alignment.sam");
#       system("$bwa_bin aln -t $thread_no -n 0.1 -o 5 -e 5 -d 5 -i 3 -k 4 -R 22  hg19+virus.fa $unmappedfq2 > unmapped.2.sai 2> /dev/null");
#       system("$bwa_bin sampe -r '$RG' hg19+virus.fa unmapped.1.sai unmapped.2.sai $unmappedfq1 $unmappedfq2 > alignment.sam 2> /dev/null");
 ...
 #      system("$bwa_bin samse -r '$RG' hg19+virus.fa unmapped.1.sai $unmappedfq1 > alignment.sam 2> /dev/null");
	print "not designed for single reads data!\n";
   }
