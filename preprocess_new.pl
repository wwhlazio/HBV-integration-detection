#get more reads unmapped to hsa
#/sc/orga/projects/zhuj05a/Wenhui/HBV/script/wgs_88/11/tumor/virusfinder/unmapped/ERR173355/aa/test: resutls of preprocess.pl comparing to ../
#!/usr/bin/perl -w
###################################################################################
#
...
sub PreprocessFastq {
    ...
    if ($paired){
       `$bowtie_bin -p $thread_no -D 5 -R 1 -N 0 -L 22 -i S,0,2.50 --end-to-end -x $bowtie_index_human -1 $fastq1 -2 $fastq2 -S $output_dir/alignment.sam`;
    }else{
       `$bowtie_bin -p $thread_no -D 5 -R 1 -N 0 -L 22 -i S,0,2.50 --end-to-end -x $bowtie_index_human -U $fastq1 -S $output_dir/alignment.sam`;
    }
